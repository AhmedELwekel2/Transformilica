...
# Azure routes by DEPLOYMENT name (what you named it in your resource), not model name.
AZURE_OPENAI_CHAT_DEPLOYMENT = os.getenv("AZURE_OPENAI_CHAT_DEPLOYMENT", "gpt-5.2-chat")
CRM_AI_CALL_TIMEOUT_SECONDS = float(os.getenv("CRM_AI_CALL_TIMEOUT_SECONDS", "11"))
CRM_AI_TIMEOUT_MAX_ATTEMPTS = max(1, int(os.getenv("CRM_AI_TIMEOUT_MAX_ATTEMPTS", "2")))
...
def _run_chat_completion_with_timeout(client: AzureOpenAI, *, messages, model: str) -> Any:
    last_error: Optional[Exception] = None
    for attempt in range(1, CRM_AI_TIMEOUT_MAX_ATTEMPTS + 1):
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            future = executor.submit(client.chat.completions.create, model=model, messages=messages)
            try:
                return future.result(timeout=CRM_AI_CALL_TIMEOUT_SECONDS)
            except concurrent.futures.TimeoutError:
                future.cancel()
                last_error = TimeoutError(
                    f"Azure OpenAI call timed out after {CRM_AI_CALL_TIMEOUT_SECONDS}s "
                    f"(attempt {attempt}/{CRM_AI_TIMEOUT_MAX_ATTEMPTS})"
                )
                if attempt == CRM_AI_TIMEOUT_MAX_ATTEMPTS:
                    raise last_error
...
def _build_column_profile(df, max_samples=5):
    """Build a compact, low-PII profile of a dataframe for LLM classification."""
    columns = []
    for col in df.columns:
        series = df[col]
        samples = [str(v)[:50] for v in series.dropna().unique()[:max_samples]]
        columns.append({
            "name": str(col),
    ...
    return {"total_rows": int(len(df)), "columns": columns}


def detect_column_roles_llm(df):
    """Use the LLM to assign semantic roles to columns for RFM and market-basket
    analysis. Returns a dict of role -> [validated column names]. Raises on any
    failure so callers can fall back to keyword heuristics."""
    profile = _build_column_profile(df)

    system_message = (
        "You are a data column classifier for CRM analytics. Given a dataset "
        "profile, assign each relevant column a semantic role. Base your decision "
        "on both the column names and the sample values. Return ONLY valid JSON."
    )
    user_content = f"""Classify the columns in this dataset.

DATASET PROFILE:
{json.dumps(profile, indent=2)}

Return a single JSON object with EXACTLY these keys. Each value is a list of exact
column names (empty list if none apply), except data_shape which is a string:
{{
  "customer_id_columns": [],
  "date_columns": [],
  "monetary_columns": [],
  ...
  "order_id_columns": [],
  "item_columns": [],
  "data_shape": "one_row_per_transaction_line OR one_row_per_entity"
}}

Definitions:
- customer_id_columns: identifies the entity a row belongs to — customer / student / patient / member (the same entity can repeat across many rows).
- date_columns: transaction / order / purchase / enrollment date-time columns.
- monetary_columns: a money AMOUNT already representing a total (line total, revenue, spend).
- unit_price_columns: a PER-ITEM price (use this, not monetary, when a separate quantity exists).
- quantity_columns: units purchased.
- order_id_columns: an invoice / order / basket / transaction id that groups the line items of a single purchase (may be absent in non-retail data).
- item_columns: the product / course / service / item the entity acquired (e.g. product name, description, SKU, course name, certification).

Rules:
- Use EXACT column names from the profile above. Never invent names.
- Leave a list empty when unsure. A column may appear in more than one list only if it genuinely fits."""
    ...
    list_keys = [
        "customer_id_columns", "date_columns", "monetary_columns",
        "unit_price_columns", "quantity_columns", "order_id_columns", "item_columns",
    ]
    roles = {key: [c for c in parsed.get(key, []) if c in valid] for key in list_keys}
    roles["data_shape"] = str(parsed.get("data_shape", "one_row_per_entity"))
    return roles


def keyword_roles(df):
    """Keyword/heuristic column-role detection — the fallback used when the LLM is
    unavailable. Returns the same shape as detect_column_roles_llm()."""
    cols = list(df.columns)
    low = {c: c.lower() for c in cols}
    n = len(df)

    def find(keywords):
        return [c for c in cols if any(k in low[c] for k in keywords)]

    date_cols = []
    for c in find(['date', 'time', 'purchase', 'order', 'transaction', 'created', 'updated', 'last', 'enroll']):
        try:
            parsed = pd.to_datetime(df[c].dropna().head(100), errors='coerce')
            if len(parsed) and parsed.notna().sum() > len(parsed) * 0.5:
                date_cols.append(c)
        except Exception:
            pass

    numeric = list(df.select_dtypes(include=[np.number]).columns)
    monetary_cols = [c for c in numeric if any(k in low[c] for k in
                     ['amount', 'price', 'revenue', 'sales', 'value', 'total', 'cost', 'payment', 'monetary', 'spend', 'spent'])]

    strong = ['customer', 'client', 'user', 'member', 'account', 'buyer', 'shopper', 'student', 'patient', 'subscriber']
    strong_ids = [c for c in cols if any(k in low[c] for k in strong)]
    weak_ids = [c for c in cols if 'id' in low[c] and c not in strong_ids
                and (df[c].nunique() / n if n else 1) < 0.98]

    return {
        'customer_id_columns': (strong_ids + weak_ids)[:3],
        'date_columns': date_cols[:3],
        'monetary_columns': monetary_cols[:3],
        'unit_price_columns': [],
        'quantity_columns': [],
        'order_id_columns': find(['invoice', 'order', 'transaction', 'receipt', 'basket', 'cart', 'bill'])[:3],
        'item_columns': find(['description', 'product', 'item', 'sku', 'stockcode', 'stock_code', 'name',
                              'title', 'course', 'class', 'subject', 'specialty', 'certification',
                              'module', 'program', 'training', 'lesson', 'service'])[:3],
        'data_shape': 'unknown',
    }


def pattern_roles(df):
    """LLM column-role detection with keyword fallback (single entry point)."""
    try:
        return detect_column_roles_llm(df)
    except Exception as e:
        print(f"LLM role detection failed ({e}); using keyword fallback")
        return keyword_roles(df)


def select_readable_item_column(df, item_cols, exclude=frozenset()):
    """Pick the most human-readable item column: a product NAME/description over a
    code/SKU, using name hints plus average text length (names run longer) as a
    tiebreaker. Falls back to the first item column when nothing scores."""
    strong_product_hints = ['description', 'product', 'item', 'title']
    candidates = list(dict.fromkeys(list(item_cols) + [
        c for c in df.columns
        if c not in exclude and any(h in c.lower() for h in strong_product_hints)
    ]))
    candidates = [c for c in candidates if c in df.columns and c not in exclude]
    if not candidates:
        return item_cols[0] if item_cols else None

    name_hints = ['name', 'description', 'title', 'label', 'product']
    code_hints = ['code', 'sku', 'stock', 'id', 'barcode', 'ref', 'no', 'num']
    ...
    def score(col):
        ...
        try:
            ...
            if len(sample):
                s += min(sample.map(len).mean(), 30) / 10.0
        except Exception:
            pass
        return s
...
class UniversalMarketBasketAnalyzer:
    def __init__(self, df, schema_analysis, transactions=None):
        self.df = df
        self.schema_analysis = schema_analysis
        self.transaction_fields = schema_analysis.get('transaction_fields', [])
        # Callers may pass pre-built baskets to skip a redundant role-detection call.
        self.transactions = transactions if transactions is not None else self.prepare_universal_transactions()
        self.item_to_index = {}
        self.index_to_item = {}
        self._build_item_index()

    def _keyword_roles_fallback(self):
        return keyword_roles(self.df)

    def _select_item_column(self, item_cols, exclude=frozenset()):
        return select_readable_item_column(self.df, item_cols, exclude)

    def _prepare_grouped_transactions(self):
        """Build baskets from line-item data by grouping an order/invoice column and
        collecting the item column per group. Returns [] if the layout doesn't fit
        (e.g. no order/item columns, or each order spans only one row)."""
        try:
            roles = detect_column_roles_llm(self.df)
        except Exception as e:
            print(f"LLM role detection failed for pattern analysis ({e}); using keyword fallback")
            roles = self._keyword_roles_fallback()

        item_cols = [c for c in roles.get('item_columns', []) if c in self.df.columns]
        # Basket key: a single order/invoice if present, else the entity the items
        # belong to (customer / student / patient). This makes both "items in one
        # invoice" and "courses one student took" valid baskets.
        group_cols = (
            [c for c in roles.get('order_id_columns', []) if c in self.df.columns]
            or [c for c in roles.get('customer_id_columns', []) if c in self.df.columns]
        )
        if not group_cols or not item_cols:
            return []

        group_col = group_cols[0]
        # Prefer a human-readable product name over a bare code/SKU when both exist,
        # so rules read as "Wireless Mouse" instead of "P1023".
        item_col = self._select_item_column(
            item_cols,
            exclude=set(group_cols) | set(roles.get('date_columns', [])),
        )

        grouped = (
            self.df.dropna(subset=[group_col, item_col])
            .groupby(group_col)[item_col]
            .apply(lambda s: sorted({str(v).strip() for v in s if str(v).strip()}))
        )
        # Keep only multi-item baskets; if the key doesn't span multiple rows this is empty.
        return [items for items in grouped if len(items) > 1]

    def prepare_universal_transactions(self):
        # Prefer invoice/order-grouped baskets (transaction-line data); fall back to
        # single-cell comma-separated baskets (one-row-per-entity data).
        grouped_transactions = self._prepare_grouped_transactions()
...
class SmartRAGAssistant:
    ...
    def answer_question(self, question):
        try:
            ...
            # Combine static template and variable data for user message
            ...
            if "rate_limit" in error_msg.lower() or "quota" in error_msg.lower():
                return self.fallback_answer(question, "API rate limit exceeded")
            elif "authentication" in error_msg.lower() or "unauthorized" in error_msg.lower():
                return self.fallback_answer(question, "API authentication failed")
            elif "invalid" in error_msg.lower() or "bad_request" in error_msg.lower():
                return self.fallback_answer(question, f"Invalid API request: {error_msg}")
            else:
                return self.fallback_answer(question, f"API error: {error_msg}")
...
def generate_dashboard_insights(df, schema_analysis):
    try:
        ...
        ai_insights_raw = parse_dashboard_response(response.choices[0].message.content)
        ...
        error_msg = str(e)
        print(f"Dashboard generation failed: {error_msg}")
        if "rate_limit" in error_msg.lower() or "quota" in error_msg.lower():
            print("API rate limit exceeded for dashboard generation")
        elif "authentication" in error_msg.lower() or "unauthorized" in error_msg.lower():
            print("API authentication failed for dashboard generation")
        elif "invalid" in error_msg.lower() or "bad_request" in error_msg.lower():
            print(f"Invalid API request for dashboard: {error_msg}")
        else:
            print(f"API error for dashboard: {error_msg}")
...
def detect_rfm_columns(df):
    """Detect suitable columns for RFM analysis.

    Tries LLM-based semantic role detection first; on any failure falls back to
    keyword heuristics. Also returns quantity/unit-price candidates so callers can
    synthesize a line total when there is no direct money column."""

    # --- Primary: LLM-based role detection ---
    try:
        roles = detect_column_roles_llm(df)
        if roles.get('customer_id_columns') or roles.get('date_columns'):
            return {
                'date_columns': roles.get('date_columns', [])[:3],
                'monetary_columns': roles.get('monetary_columns', [])[:3],
                'customer_id_columns': roles.get('customer_id_columns', [])[:3],
                'quantity_columns': roles.get('quantity_columns', [])[:3],
                'unit_price_columns': roles.get('unit_price_columns', [])[:3],
                'detection_method': 'llm',
            }
    except Exception as e:
        print(f"LLM RFM column detection failed ({e}); using keyword fallback")
    ...
    # Detect date columns
    date_keywords = ['date', 'time', 'purchase', 'order', 'transaction', 'created', 'updated', 'last']
    for col in df.columns:
        col_lower = col.lower()
        if any(keyword in col_lower for keyword in date_keywords):
            try:
                # Try to parse as date
                test_series = pd.to_datetime(df[col].dropna().head(100), errors='coerce')
                if test_series.notna().sum() > len(test_series) * 0.5:  # At least 50% parseable
                    date_columns.append(col)
            except:
                pass
    
    # If no date columns found by keyword, try all columns
    if not date_columns:
        for col in df.columns:
            try:
                test_series = pd.to_datetime(df[col].dropna().head(100), errors='coerce')
                if test_series.notna().sum() > len(test_series) * 0.5:
                    date_columns.append(col)
            except:
                pass
    
    # Detect monetary columns
    monetary_keywords = ['amount', 'price', 'revenue', 'sales', 'value', 'total', 'cost', 'payment', 'monetary', 'spend', 'spent']
    numeric_cols = df.select_dtypes(include=[np.number]).columns
    for col in numeric_cols:
        col_lower = col.lower()
        if any(keyword in col_lower for keyword in monetary_keywords):
            # Check if values are positive (typical for monetary)
            non_negative_ratio = (df[col].dropna() >= 0).sum() / df[col].dropna().count() if df[col].dropna().count() > 0 else 0
            if non_negative_ratio > 0.7:  # At least 70% non-negative
                monetary_columns.append(col)
    
    # If no monetary columns found by keyword, use first numeric column with positive values
    if not monetary_columns:
        for col in numeric_cols:
            non_negative_ratio = (df[col].dropna() >= 0).sum() / df[col].dropna().count() if df[col].dropna().count() > 0 else 0
            if non_negative_ratio > 0.7:
                monetary_columns.append(col)
                break
    
    # Detect customer ID columns by name. Prefer explicit customer-like names over a
    # generic "id" match, and do NOT reject a column for repeating a lot: in
    # transaction-line data the real customer id appears across many rows.
    strong_keywords = ['customer', 'client', 'user', 'member', 'account', 'buyer', 'shopper']
    strong_ids = [c for c in df.columns if any(k in c.lower() for k in strong_keywords)]
    weak_ids = [c for c in df.columns if 'id' in c.lower() and c not in strong_ids]
    # From the generic "id" group only, drop near-unique columns (likely a row key).
    filtered_weak = []
    for col in weak_ids:
        unique_ratio = df[col].nunique() / len(df) if len(df) > 0 else 0
        if unique_ratio < 0.98:
            filtered_weak.append(col)
    customer_id_columns = strong_ids + filtered_weak

    return {
        'date_columns': date_columns[:3],  # Return top 3 candidates
        'monetary_columns': monetary_columns[:3],
        'customer_id_columns': customer_id_columns[:3],
        'quantity_columns': [],
        'unit_price_columns': [],
        'detection_method': 'keyword',
    }

# Targeting priority for offers: higher = contact sooner. High-value and
# slipping-away segments rank above already-loyal or lost ones.
SEGMENT_PRIORITY = {
    'Champions': 95,
    'Cannot Lose Them': 92,
    'At Risk': 88,
...
def _assign_segment(r, f, m):
    """Map R/F/M scores (1-5) to a segment name (same rules as perform_rfm_analysis)."""
...
def compute_customer_segments(df, date_column, customer_id_column, monetary_column):
    """Return {customer_id: {segment, recency, frequency, monetary}} via RFM scoring.
    Used to prioritize which customers to send offers to."""
    try:
        d = df.copy()
        d[date_column] = pd.to_datetime(d[date_column], errors='coerce')
        d = d.dropna(subset=[date_column, customer_id_column, monetary_column])
        if len(d) == 0:
            return {}
        max_date = d[date_column].max()
        g = d.groupby(customer_id_column)
        rfm = pd.DataFrame({
            'Recency': (max_date - g[date_column].max()).dt.days,
            'Frequency': g[date_column].count(),
            'Monetary': g[monetary_column].sum(),
        })
        rfm['R'] = pd.to_numeric(pd.qcut(rfm['Recency'].rank(method='first'), 5, labels=[5, 4, 3, 2, 1], duplicates='drop'), errors='coerce')
        rfm['F'] = pd.to_numeric(pd.qcut(rfm['Frequency'].rank(method='first'), 5, labels=[1, 2, 3, 4, 5], duplicates='drop'), errors='coerce')
        rfm['M'] = pd.to_numeric(pd.qcut(rfm['Monetary'].rank(method='first'), 5, labels=[1, 2, 3, 4, 5], duplicates='drop'), errors='coerce')
        out = {}
        for cid, row in rfm.iterrows():
            out[str(cid)] = {
                'segment': _assign_segment(row['R'], row['F'], row['M']),
                'recency': int(row['Recency']) if pd.notna(row['Recency']) else None,
        ...
        print(f"Segment computation failed: {e}")
        return {}


def perform_rfm_analysis(df, date_column, customer_id_column, monetary_column):
    ...
    try:
        ...
        # Parse date column
        rfm_df[date_column] = pd.to_datetime(rfm_df[date_column], errors='coerce')
        
        # Remove rows with missing critical data
        rfm_df = rfm_df.dropna(subset=[date_column, customer_id_column, monetary_column])
        
        if len(rfm_df) == 0:
            return None, "No valid records found after removing missing data"
        
        # Get the most recent date in the dataset
        max_date = rfm_df[date_column].max()
        
        # Calculate Recency (days since last purchase)
        recency_df = rfm_df.groupby(customer_id_column).agg({
            date_column: 'max'
        }).reset_index()
        recency_df['Recency'] = (max_date - recency_df[date_column]).dt.days
        
        # Calculate Frequency (number of transactions)
        frequency_df = rfm_df.groupby(customer_id_column).agg({
            date_column: 'count'
        }).reset_index()
        frequency_df.columns = [customer_id_column, 'Frequency']
        
        # Calculate Monetary (total amount spent)
        monetary_df = rfm_df.groupby(customer_id_column).agg({
            monetary_column: 'sum'
        }).reset_index()
        monetary_df.columns = [customer_id_column, 'Monetary']
        
        # Merge all RFM metrics
        rfm_metrics = recency_df.merge(frequency_df, on=customer_id_column)
        rfm_metrics = rfm_metrics.merge(monetary_df, on=customer_id_column)
        rfm_metrics = rfm_metrics.drop(columns=[date_column])
        
        # Calculate RFM scores (1-5 scale, where 5 is best)
        # Recency: Lower days = Higher score (more recent = better)
        rfm_metrics['R_Score'] = pd.qcut(rfm_metrics['Recency'].rank(method='first'), q=5, labels=[5, 4, 3, 2, 1], duplicates='drop')
        
        # Frequency: Higher frequency = Higher score
        rfm_metrics['F_Score'] = pd.qcut(rfm_metrics['Frequency'].rank(method='first'), q=5, labels=[1, 2, 3, 4, 5], duplicates='drop')
        
        # Monetary: Higher amount = Higher score
        rfm_metrics['M_Score'] = pd.qcut(rfm_metrics['Monetary'].rank(method='first'), q=5, labels=[1, 2, 3, 4, 5], duplicates='drop')
        
        # Convert scores to numeric
        rfm_metrics['R_Score'] = pd.to_numeric(rfm_metrics['R_Score'], errors='coerce')
        rfm_metrics['F_Score'] = pd.to_numeric(rfm_metrics['F_Score'], errors='coerce')
        rfm_metrics['M_Score'] = pd.to_numeric(rfm_metrics['M_Score'], errors='coerce')
        ...
        def assign_segment(row):
            r = float(row['R_Score']) if pd.notna(row['R_Score']) else 0.0
            f = float(row['F_Score']) if pd.notna(row['F_Score']) else 0.0
            m = float(row['M_Score']) if pd.notna(row['M_Score']) else 0.0
        ...
        segment_stats.columns = ['Segment', 'Customer_Count', 'Avg_Recency_Days', 'Avg_Frequency', 'Avg_Monetary']
        segment_stats = segment_stats.sort_values('Customer_Count', ascending=False)
        
        # Prepare response data - convert all numpy/pandas types to native Python types
        total_customers = int(len(rfm_metrics))
        segment_summary = []
        for _, row in segment_stats.iterrows():
            customer_count = int(row['Customer_Count']) if pd.notna(row['Customer_Count']) else 0
            avg_recency = float(row['Avg_Recency_Days']) if pd.notna(row['Avg_Recency_Days']) else 0.0
            avg_frequency = float(row['Avg_Frequency']) if pd.notna(row['Avg_Frequency']) else 0.0
            avg_monetary = float(row['Avg_Monetary']) if pd.notna(row['Avg_Monetary']) else 0.0
            ...
            segment_summary.append({
                ...
                'avgRecencyDays': round(avg_recency, 1),
                'avgFrequency': round(avg_frequency, 2),
                'avgMonetary': round(avg_monetary, 2)
            })
        
        # Top customers by RFM score
        top_customers = rfm_metrics.nlargest(10, 'Monetary')[[customer_id_column, 'Recency', 'Frequency', 'Monetary', 'Segment']]
        top_customers_list = []
        for _, row in top_customers.iterrows():
            ...
            recency_days = int(recency_val) if pd.notna(recency_val) else 0
            frequency = int(frequency_val) if pd.notna(frequency_val) else 0
            monetary = float(monetary_val) if pd.notna(monetary_val) else 0.0
            customer_id = str(row[customer_id_column]) if pd.notna(row[customer_id_column]) else ''
        ...
        # Calculate overall stats and convert to native Python types
        avg_recency = float(rfm_metrics['Recency'].mean()) if pd.notna(rfm_metrics['Recency'].mean()) else 0.0
        avg_frequency = float(rfm_metrics['Frequency'].mean()) if pd.notna(rfm_metrics['Frequency'].mean()) else 0.0
        avg_monetary = float(rfm_metrics['Monetary'].mean()) if pd.notna(rfm_metrics['Monetary'].mean()) else 0.0
        total_revenue = float(rfm_metrics['Monetary'].sum()) if pd.notna(rfm_metrics['Monetary'].sum()) else 0.0
        ...
        return None, f"Error performing RFM analysis: {str(e)}"

@app.route('/ai/rfm-analysis', methods=['POST'])
def rfm_analysis():
    try:
        if 'data' not in request.files:
            return jsonify({
        ...
        # Detect RFM columns
        rfm_columns = detect_rfm_columns(df)
        
        # Check if we have required columns
        date_cols = rfm_columns.get('date_columns', [])
        monetary_cols = rfm_columns.get('monetary_columns', [])
        customer_id_cols = rfm_columns.get('customer_id_columns', [])
        quantity_cols = rfm_columns.get('quantity_columns', [])
        unit_price_cols = rfm_columns.get('unit_price_columns', [])

        # No direct money column but we have quantity + unit price → synthesize a
        # per-line revenue column (e.g. Quantity x UnitPrice on transaction data).
        if not monetary_cols and quantity_cols and unit_price_cols:
            qty_col, price_col = quantity_cols[0], unit_price_cols[0]
            df['line_total'] = (
                pd.to_numeric(df[qty_col], errors='coerce') *
                pd.to_numeric(df[price_col], errors='coerce')
            )
            monetary_cols = ['line_total']

        if not date_cols:
            return jsonify({
                "status": "error",
                "message": "No suitable date column found for RFM analysis. RFM analysis requires a date/time column to calculate recency.",
                "suggestions": [
                    "Ensure your dataset contains a column with dates (e.g., purchase_date, order_date, transaction_date)",
                    "Date columns should be in a recognizable format (YYYY-MM-DD, MM/DD/YYYY, etc.)",
                    "Column names containing 'date', 'time', 'purchase', 'order', or 'transaction' are preferred"
                ],
                "detectedColumns": {
                    "dateColumns": [],
                    "monetaryColumns": monetary_cols,
                    "customerIdColumns": customer_id_cols
                }
            }), 200
        ...
        if not monetary_cols:
            return jsonify({
                "status": "error",
                "message": "No suitable monetary column found for RFM analysis. RFM analysis requires a numeric column representing transaction amounts.",
                "suggestions": [
                    "Ensure your dataset contains a numeric column with transaction amounts (e.g., amount, price, revenue, sales)",
                    "Monetary columns should contain positive numeric values",
                    "Column names containing 'amount', 'price', 'revenue', 'sales', or 'value' are preferred"
                ],
                "detectedColumns": {
        ...
        if not customer_id_cols:
            return jsonify({
                "status": "error",
                "message": "No suitable customer ID column found for RFM analysis. RFM analysis requires a column to identify unique customers.",
                "suggestions": [
                    "Ensure your dataset contains a column identifying customers (e.g., customer_id, client_id, user_id)",
                    "Customer ID columns should have reasonable uniqueness (not too unique like sequential IDs, not too few unique values)",
                    "Column names containing 'customer', 'client', 'user', or 'id' are preferred"
                ],
                "detectedColumns": {
                    "dateColumns": date_cols,
                    "monetaryColumns": monetary_cols,
        ...
        monetary_column = monetary_cols[0]
        customer_id_column = customer_id_cols[0]
        
        # Perform RFM analysis
        rfm_result, error_message = perform_rfm_analysis(df, date_column, customer_id_column, monetary_column)
        
        if rfm_result is None:
            return jsonify({
                "status": "error",
                "message": error_message or "RFM analysis could not be completed",
                "columnsUsed": {
                    "dateColumn": date_column,
                    "monetaryColumn": monetary_column,
        ...
        response_data = {
            ...
            "alternativeColumns": {
                "dateColumns": date_cols[1:] if len(date_cols) > 1 else [],
                "monetaryColumns": monetary_cols[1:] if len(monetary_cols) > 1 else [],
                "customerIdColumns": customer_id_cols[1:] if len(customer_id_cols) > 1 else []
            }
        }
        ...
        print(f"Unexpected error in RFM analysis: {str(e)}")
        return jsonify({
            "status": "error",
            "message": f"Unexpected error: {str(e)}"
        }), 500
...
def recommendations():
    """Per-customer product recommendations from market-basket rules, prioritized by
    RFM segment. Output: customerId -> top-N recommended products (with why)."""
    try:
        if 'data' not in request.files:
        ...
        min_support = float(request.form.get('min_support', 0.02))
        min_confidence = float(request.form.get('min_confidence', 0.3))
        seg_arg = request.form.get('segments')  # optional CSV of segment names to keep
        segment_filter = {s.strip() for s in seg_arg.split(',')} if seg_arg else None
        ...
        # One role-detection call drives everything below.
        roles = pattern_roles(df)
        customer_cols = [c for c in roles.get('customer_id_columns', []) if c in df.columns]
        item_cols = [c for c in roles.get('item_columns', []) if c in df.columns]
        if not customer_cols or not item_cols:
            return jsonify({
                "status": "error",
                "message": "Recommendations need both a customer column and a product/item column.",
                "detected": {"customerColumns": customer_cols, "itemColumns": item_cols}
            }), 200

        customer_col = customer_cols[0]
        item_col = select_readable_item_column(
            df, item_cols, exclude=set(customer_cols) | set(roles.get('date_columns', [])))

        # Basket key for mining rules: an order/invoice if present, else the customer.
        group_cols = ([c for c in roles.get('order_id_columns', []) if c in df.columns] or [customer_col])
        group_col = group_cols[0]

        baskets = (df.dropna(subset=[group_col, item_col])
                     .groupby(group_col)[item_col]
                     .apply(lambda s: sorted({str(v).strip() for v in s if str(v).strip()})))
        transactions = [b for b in baskets if len(b) > 1]
        if not transactions:
            return jsonify({"status": "error",
                            "message": "No multi-item baskets found — cannot mine recommendations."}), 200

        schema_analysis = DataSchemaAnalyzer(df).analyze_schema()
        analyzer = UniversalMarketBasketAnalyzer(df, schema_analysis, transactions=transactions)
        _, rules = analyzer.analyze_patterns(
            min_support=min_support, min_confidence=min_confidence, max_itemset_size=3)

        if not rules:
            return jsonify({
                "status": "success",
                "message": f"No association rules met support {min_support} / confidence {min_confidence}. Try lowering them.",
                "recommendations": []
            }), 200

        prepared_rules = [(set(r['antecedent']), r['consequent'], r['confidence'], r['lift']) for r in rules]

        # Each customer's owned items (union across all their rows/invoices).
        owned = (df.dropna(subset=[customer_col, item_col])
                   .groupby(customer_col)[item_col]
                   .apply(lambda s: {str(v).strip() for v in s if str(v).strip()}))

        # RFM segment per customer (needs a date + a money column; synth if needed).
        ...
        segments = compute_customer_segments(df, date_cols[0], customer_col, monetary_col) \
            if (date_cols and monetary_col) else {}

        results = []
        for cid, items in owned.items():
            recs = {}
            for ante, cons_list, conf, lift in prepared_rules:
                if ante and ante.issubset(items):
                    for cons in cons_list:
                        if cons not in items:
                            sc = conf * lift
                            if cons not in recs or sc > recs[cons]['_score']:
                                recs[cons] = {"product": cons, "confidence": round(conf, 3),
                                              "lift": round(lift, 2), "because": sorted(ante), "_score": sc}
            if not recs:
                continue
            top = sorted(recs.values(), key=lambda x: x['_score'], reverse=True)[:top_n]
            for t in top:
                t.pop('_score', None)
            seg = segments.get(str(cid), {})
            segment_name = seg.get('segment')
            if segment_filter and segment_name not in segment_filter:
                continue
            results.append({
                "customerId": str(cid),
                "segment": segment_name,
                "monetary": seg.get('monetary'),
                "priority": SEGMENT_PRIORITY.get(segment_name, 0),
                "recommendations": top,
            })

        # Best targets first: by segment priority, then by customer value.
        results.sort(key=lambda x: (x['priority'], x['monetary'] or 0), reverse=True)
        ...
        return jsonify({
            ...
            "rulesMined": len(rules),
            "totalCustomersWithRecommendations": len(results),
